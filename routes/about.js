...
router.get('/luthiers',
	...
	async (req, res, next) => {
		...
		// get the chapter names for populating the navbar
		const chapter_names_query = `SELECT id AS chapter_id, name, address FROM chapters ORDER BY chapter_id`
		// get the entire roster for populating the navbar
		...
		const {address = 'Eagle Rock, MO'} = chapter_names[output_query.chapter - 1]
